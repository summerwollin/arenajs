...
io.on('connection', function(socket){
  ...
  socket.on('disconnect', function (msg) {
    ...
    hostedGames.forEach(function (hostedGame, i) {
      if (hostedGame.hostUser === session.username) {
        console.log('HOST USER HAS DISCONNECTED', hostedGames);
  ...
  socket.on('join-game', function (msg) {
    console.log('io [join-game]', msg);
    io.emit('join-game', msg);
  })
  socket.on('answer', function(msg) {
    console.log('io [answer]', msg);
    io.emit('answer', msg);
  })
